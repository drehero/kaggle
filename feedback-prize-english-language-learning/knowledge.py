...
pos_map = dict(zip(pos_tags, range(len(pos_tags))))
...
dependency_map = dict(zip(dependency_labels, range(len(dependency_labels))))
...
entity_map = dict(zip(named_entities, range(len(named_entities))))

# https://github.com/andreasvc/readability/blob/master/readability/langdata.py
...
""".lower().split()
dale_chall_map = dict(zip(basicwords_en, range(len(basicwords_en))))

n_knowledge_dims = sum([len(pos_map), len(entity_map), len(dependency_map)]) + 2  # 2 dimesnions for dale chall: is basic word or not
